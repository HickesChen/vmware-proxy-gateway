...
- 提供独立流量日志页，可按目标、端口和代理/直连路由筛选 sing-box 的真实转发记录。
...
2. 填写 Windows 主机代理 IP 和一个与你的 Windows 代理配置一致的端口。
...
如果 Ubuntu 无法连接所配置的 TCP 端口，请检查 Windows 代理软件：

- 开启 LAN access / allow LAN。
- 绑定到 `0.0.0.0` 或 Windows 的 LAN/VMware IP，不要只监听 `127.0.0.1`。
- 在 Windows Defender 防火墙里允许专用网络入站所配置的端口。
...
GUI 现在使用一张表统一管理自定义绕过规则。点击“添加规则”可以选择 IP / CIDR、
域名完全匹配、前缀、后缀、包含关键词或正则表达式，也可以使用“取反（非）”。
旧版本中已有的 CIDR 和域名会自动合并到规则表，不需要重新录入。
规则表同时显示系统保护规则、当前代理、默认网关、虚拟机本地地址/网段和已应用预设；
这些自动规则标记为只读，自定义规则仍可编辑和删除。
...
IPv4 通配符必须从某个完整八位组开始并连续写到末尾。例如 `192.168.*.*` 等价于 `192.168.0.0/16`，`192.*.1.*` 这类不连续写法不会被接受。
